        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center">
            ...
            <div className="flex-1 overflow-hidden">
              <div className="animate-marquee whitespace-nowrap">
                <span className="text-white font-medium text-lg">
                  🎉 {currentWinner.username} {t('winners.won')} {formatAmount(currentWinner.amount)} - {currentWinner.game} {t('winners.in')} ({getTimeAgo(currentWinner.timestamp)})
                  <span className="mx-12 text-yellow-300">•</span>
                  {recentWinners[(currentIndex + 1) % recentWinners.length] && (
                    <>
                      🎊 {recentWinners[(currentIndex + 1) % recentWinners.length].username} {t('winners.won')} {formatAmount(recentWinners[(currentIndex + 1) % recentWinners.length].amount)} - {recentWinners[(currentIndex + 1) % recentWinners.length].game} {t('winners.in')}
                      <span className="mx-12 text-yellow-300">•</span>
                    </>
                  )}
                  {recentWinners[(currentIndex + 2) % recentWinners.length] && (
                    <>
                      🏆 {recentWinners[(currentIndex + 2) % recentWinners.length].username} {t('winners.won')} {formatAmount(recentWinners[(currentIndex + 2) % recentWinners.length].amount)} - {recentWinners[(currentIndex + 2) % recentWinners.length].game} {t('winners.in')}
                    </>
                  )}
                </span>
              </div>
