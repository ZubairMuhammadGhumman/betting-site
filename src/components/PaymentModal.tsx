  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!amountNum || amountNum <= 0) {
        setError(t('error.amountRequired'));
        return;
      }
      ...
      if (selectedMethod !== 'c2c') {
        if (!cardNumber || !expiryDate || !cvv) {
          setError(t('error.cardDetailsRequired'));
          return;
        }
      ...
      const errorMessage = error.response?.data?.message || t('error.paymentFailed');
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-2xl font-bold text-gray-800">
              {type === 'deposit' ? t('payment.deposit') : t('payment.withdraw')}
            </h2>
          </div>
          <button
            onClick={handleClose}
              ...
              {savedCards.length > 0 && (
                <div className="space-y-3 mb-6">
                  <h4 className="text-sm font-medium text-gray-600 flex items-center space-x-2">
                    ...
                    <span>{t('payment.savedCards')}</span>
                  </h4>
                  {savedCards.map((card: any) => (
                    ...
                    >
                      ...
                      <div className="flex-1 text-left">
                        ...
                        <p className="text-xs text-green-600 font-medium">{t('payment.readyToUse')}</p>
                      </div>
                  ...
                  <div className="border-t border-gray-200 my-4 pt-4">
                    <p className="text-sm text-gray-500 text-center mb-3">{t('payment.addNewCard')}</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-xs text-green-600 font-medium">{t('payment.minimum')}: 10 AZN</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-xs text-green-600 font-medium">{t('payment.minimum')}: 5 AZN</p>
                  </div>
                </button>
              </div>
            </div>
              ...
              <div className="flex items-center space-x-2 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-800">
                    {selectedMethod?.toUpperCase()} {type === 'deposit' ? t('payment.deposit') : t('payment.withdraw')}
                  </h3>
                  {selectedSavedCard && (
                    <p className="text-sm text-blue-600">{t('payment.readyToUse')}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                {type === 'deposit' && (
                  <p className="text-xs text-gray-500 mt-1">
                    {t('payment.minimum')}: {selectedMethod === 'c2c' ? '5' : '10'} AZN, {t('payment.maximum')}: 50,000 AZN
                  </p>
                )}
                {type === 'withdraw' && (
                  <p className="text-xs text-gray-500 mt-1">
                    {t('payment.minimum')}: {selectedMethod === 'c2c' ? '5' : '10'} AZN, {t('payment.maximum')}: 10,000 AZN
                  </p>
                )}
              </div>
              ...
              {selectedMethod !== 'c2c' && (
                <>
                  ...
                  <div>
                    ...
                    />
                    {selectedSavedCard && (
                      <p className="text-xs text-blue-600 mt-1">{t('payment.readyToUse')}</p>
                    )}
                  </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="text-sm text-blue-700">
                      <p className="font-medium mb-1">{t('payment.c2cInstructions')}</p>
                      <p>{t('payment.c2cDesc')}</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          {step === 'success' && (
            <div className="text-center space-y-4">
              ...
              <h3 className="text-2xl font-bold text-gray-800">
                {type === 'deposit' ? t('payment.depositSuccess') : t('payment.withdrawSuccess')}
              </h3>
              <p className="text-gray-600">
                {amount} AZN məbləğində {type === 'deposit' ? t('payment.deposit').toLowerCase() : t('payment.withdraw').toLowerCase()} uğurla yaradıldı.
              </p>
              {transactionResult?.transactionId && (
                <p className="text-sm text-gray-500">
                  {t('payment.transactionId')}: {transactionResult.transactionId}
                </p>
              )}
              <button
